...
> **作者确认（2026-07）：** 这是 rynk-wasm 的误设计，不是有意约束。`await next_event()` 原本就不应该阻塞 `edit`/`set_*()`——并发双循环是预期架构。本文记录的是 bug 现状而非设计约束；rynk-wasm 修复后 rmk-gui 可直接采用独立 topic loop + sync processor 并发架构，无需 workaround。

## 问题

`RynkClient`（WASM 导出）的所有方法都取 `&mut self`。`next_event()` 和 `set_*()` 不能并发调用，否则 client 永久死亡。**此行为是 bug。**
...
> 本节描述的是 workaround 现状。rynk-wasm 修复后，此 workaround 可移除，直接采用独立 topic loop + sync processor 并发架构。
...
### 方案 A：修复 rynk-wasm（推荐，作者已确认是误设计）

**这是正确方向。** 作者确认并发是预期行为，修复后 rmk-gui 无需 workaround。
...
### 方案 C：rmk-gui 侧单循环（当前 workaround）
