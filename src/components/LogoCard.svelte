<script lang='ts'>
  ...
  function pickDevice(kind: 'serial' | 'hid') {
    open = false
    void deviceStore.pick(kind)
  }
</script>
...
<div
  class='relative flex h-full flex-none items-center gap-2.5'
  role='presentation'
...
>
  ...
  {#if open}
    <div
      class={`
        absolute top-[calc(100%+6px)] left-0 z-70 flex w-[268px] flex-col
        gap-0.5 rounded-lg border border-border bg-popover p-1.5 shadow-lg
      `}
    >
      <div
        class={`
      ...
      {#each deviceStore.devices as candidate (candidate.id)}
        ...
        <button
          class={[
            `
              flex cursor-pointer items-center gap-2.5 rounded-md p-2 text-left
              transition-colors
            `,
            on ? 'bg-brand-tint' : 'hover:bg-base-200',
          ]}
          type='button'
          onclick={() => switchTo(candidate, on)}
        >
          <span
            class={`
