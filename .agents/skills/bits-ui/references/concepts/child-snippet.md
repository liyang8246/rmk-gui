...
The `child` snippet gives you complete control over which HTML element renders for a Bits UI component, while preserving all behavior, accessibility, and event handling.
...
Many Bits UI components have default HTML elements. For example, `Accordion.Trigger` renders a `<button>` by default:
...
Behind the scenes, components that support the `child` snippet implement logic similar to:
...
Apply Svelte-specific features to your custom elements:
...
## Floating Components — Special Handling

Floating content components (tooltips, popovers, dropdowns, etc.) require a **two-level structure** due to their positioning requirements.
...
- The wrapper element with `{...wrapperProps}` must remain **unstyled** — positioning is handled here
- Styling goes on the inner content element
- The `open` parameter lets you conditionally render the content, enabling Svelte transitions
- Always maintain this two-level structure to ensure proper positioning
...
### Floating Element with Transition
